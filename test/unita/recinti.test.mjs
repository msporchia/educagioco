...
/* Gli stati che il motore può davvero far vedere. `calmo` non c'è ed è
   voluto: è il ritratto del baule, dove non c'è nessun orologio da
   raccontare. In mappa un recinto o ha fame, o sta facendo, o è pronto. */
...
for (const v of RECINTI) {
  stessaLista(`${v.id}: i sei stati sono quelli attesi`,
  ...
  /* Il ritratto del baule dev'essere uno dei sei: se fosse un settimo
     disegno, si comprerebbe una cosa e in mappa ne comparirebbe
     un'altra — e nessuno collegherebbe le due. */
  controlla(`${v.id}: il ritratto del baule è uno dei sei`,
   ...
   in un millisecondo una cosa che in gioco dura un quarto d'ora. */
{
  ...
  uguale('senza grano non si avvia', niente.ok, false)
  ...
  controlla('col grano in granaio si parte', via.ok)
  uguale('e il grano se ne va tutto', f.quantoHo('grano'), 0)
