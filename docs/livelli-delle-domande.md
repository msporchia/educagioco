# I livelli delle domande

Una riga per classe (modulo · grado · tipologia), ordinate per **livello**: un
numero da 0 a 100 sulla stessa scala per tutte le materie — 0 è il primo giorno
di materna, 100 la fine della primaria, dodici punti e mezzo per anno di scuola.
Fra parentesi l'età che corrisponde, che è come si legge nella schermata dei grandi.

Il livello dice **quanto è complicata**, non a chi arriva: quello lo decide la
finestra di chi gioca (16 punti sotto la sua età, 20 sopra, allargata quando il
mazzo è magro). Le righe con ⚑ hanno un livello dichiarato sulla tipologia,
diverso da quello del suo grado.

**Questo file è generato** da `npm run quiz:livelli`: non si scrive a mano.


## 0  ·  circa 4 anni

- 🔢 **Senso del numero** g1 — Chi è di più, chi è di meno
- 🔢 **Senso del numero** g1 — Il colpo d'occhio: quanti sono
- ➡️ **Sequenze** g1 — Il ritmo che si ripete

## 12  ·  circa 5 anni

- 🦁 **Animali e ambienti** g1 — Dove vive questo animale
- 🔎 **Indizi** g1 — Chi sono io: le figure
- 🅰️ **Le prime lettere** g1 — Con che lettera comincia
- 🔢 **Senso del numero** g2 — Leggere la linea dei numeri
- 🔢 **Senso del numero** g2 — Mettere un numero al suo posto
- ➡️ **Sequenze** g2 — Chi non c'entra

## 20  ·  circa 5.6 anni

- 📅 **Calendario** g1 — I giorni della settimana
- 📐 **Geometria** g1 — Le figure e i loro nomi

## 25  ·  circa 6 anni

- 🔗 **Analogie** g1 — Le analogie sulle cose del mondo
- 🦁 **Animali e ambienti** g2 — Dove vive questo animale
- 📅 **Calendario** g2 — L'ordine dei mesi
- 📐 **Geometria** g2 — Contare lati e vertici
- 🔤 **Grammatica** g1 — Maschile e femminile
- 🔤 **Grammatica** g1 — Singolare e plurale
- 🔎 **Indizi** g2 — Indovinello: le cose del mondo
- 🗺️ **La griglia** g1 — Le caselle: lettera e numero
- 🅰️ **Le prime lettere** g2 — Con che lettera comincia
- 🅰️ **Le prime lettere** g2 — Leggere la parola, non solo la prima lettera
- 📖 **Lessico** g1 — I contrari
- 🧩 **Logica** g1 — «Nessuno» vuol dire nessuno
- 🧩 **Logica** g1 — La regola applicata dritta
- 🕰️ **Orologio** g1 — Le ore intere
- 🔢 **Senso del numero** g3 — Prima, dopo, in mezzo e in ordine
- ➡️ **Sequenze** g3 — Il ritmo che si ripete
- ✂️ **Sillabe** g1 — Quante sillabe ha la parola

## 29  ·  circa 6.3 anni

- 📅 **Calendario** g3 — Le feste dell'anno
- 📅 **Calendario** g3 — Le stagioni
- 📐 **Geometria** g3 — Dove si piega
- 📐 **Geometria** g3 — La metà che manca
- 🔤 **Grammatica** g2 — L'articolo giusto
- 🗺️ **La griglia** g2 — Destra, sinistra, sopra e sotto
- 🅰️ **Le prime lettere** g3 — Con che lettera comincia
- 🅰️ **Le prime lettere** g3 — La sillaba iniziale
- 🅰️ **Le prime lettere** g3 — Leggere la parola, non solo la prima lettera
- 📖 **Lessico** g2 — L'intruso
- 🧩 **Logica** g2 — Chi è più alto di chi

## 33  ·  circa 6.6 anni

- 🗺️ **La griglia** g3 — Dove si arriva seguendo le frecce
- 🗺️ **La griglia** g3 — Quante mosse servono

## 38  ·  circa 7 anni

- 🔗 **Analogie** g2 — Le analogie sulle cose del mondo
- 🦁 **Animali e ambienti** g3 — Chi non vive qui
- 🦁 **Animali e ambienti** g3 — Chi vive in questo posto
- 🦁 **Animali e ambienti** g3 — Dove vive questo animale
- ⚑ 📅 **Calendario** g2 — Quanti giorni ha un mese
- 🗣️ **Coniugazione** g1 — Il presente dei verbi regolari
- ⚑ 📐 **Geometria** g1 — I quadrilateri (quadrato, rombo, trapezio)
- 📐 **Geometria** g4 — Girare una figura di un quarto
- 📐 **Geometria** g4 — La figura allo specchio
- ⚑ 🔤 **Grammatica** g5 — Nomi propri e nomi comuni
- 🔎 **Indizi** g3 — Chi sono io: le figure
- 📏 **Misure** g1 — Con che cosa si misura
- 🕰️ **Orologio** g2 — Le mezze e i quarti d'ora
- 🕰️ **Orologio** g2 — Le ore intere
- ✏️ **Ortografia** g1 — Il suono gl (famiglia)
- ✏️ **Ortografia** g1 — Il suono gn (montagna)
- ⚑ ✏️ **Ortografia** g5 — La lettera h (ho, hai, ha, hanno)
- 📝 **Problemi** g1 — Quello che arriva si somma
- 🔢 **Senso del numero** g4 — Il numero più vicino
- 🔢 **Senso del numero** g4 — Leggere la linea dei numeri
- 🔢 **Senso del numero** g4 — Mettere un numero al suo posto
- ➡️ **Sequenze** g4 — Chi non c'entra
- ✂️ **Sillabe** g2 — Dividere in sillabe

## 44  ·  circa 7.5 anni

- 🔗 **Analogie** g3 — Le analogie fra figure
- 🗣️ **Coniugazione** g2 — Il presente dei verbi irregolari
- 🕰️ **Orologio** g3 — Le mezze e i quarti d'ora
- 🕰️ **Orologio** g3 — Le ore intere
- 🕰️ **Orologio** g3 — Trovare l'orologio che segna un'ora
- ✏️ **Ortografia** g2 — Il suono gl (famiglia)
- ✏️ **Ortografia** g2 — Il suono gn (montagna)
- ✏️ **Ortografia** g2 — Il suono sc (pesce, scienza)
- 📝 **Problemi** g2 — Quello che arriva si somma
- 📝 **Problemi** g2 — Quello che va via si toglie
- ✂️ **Sillabe** g3 — Le rime

## 50  ·  circa 8 anni

- 🦁 **Animali e ambienti** g4 — Chi non vive qui
- 🦁 **Animali e ambienti** g4 — Dove vive questo animale
- 🦁 **Animali e ambienti** g4 — Il corpo dice il posto
- ✏️ **Ortografia** g3 — Il suono sc (pesce, scienza)
- ✏️ **Ortografia** g3 — Le doppie
- ✂️ **Sillabe** g4 — La sillaba che manca

## 56  ·  circa 8.5 anni

- 🔗 **Analogie** g4 — Le analogie fra figure
- 📅 **Calendario** g4 — Contare i giorni fra due date
- ⚑ 🗣️ **Coniugazione** g1 — I verbi in -isc (finire, capire)
- 🗣️ **Coniugazione** g3 — Essere o avere nel passato prossimo
- 🗣️ **Coniugazione** g3 — I participi irregolari (preso, scritto)
- 🗣️ **Coniugazione** g3 — Il participio passato regolare
- ⚑ 📐 **Geometria** g2 — Gli angoli: retto, acuto, ottuso
- 📐 **Geometria** g5 — Come si vede un solido dall'alto
- 📐 **Geometria** g5 — Contare le facce
- 📐 **Geometria** g5 — I solidi e i loro nomi
- 🔤 **Grammatica** g3 — Che parte del discorso è
- 🔎 **Indizi** g4 — Indovinello: le cose del mondo
- 🗺️ **La griglia** g4 — L'area: i quadretti dentro
- 📖 **Lessico** g3 — I sinonimi
- 🧩 **Logica** g3 — La regola applicata dritta
- 🧩 **Logica** g3 — La regola girata: non si può sapere
- 🧩 **Logica** g3 — Quando la regola non è scattata
- 📏 **Misure** g2 — Quanto è grande davvero
- 🕰️ **Orologio** g4 — Che ora sarà fra un po'
- 🕰️ **Orologio** g4 — I minuti
- 🕰️ **Orologio** g4 — Le mezze e i quarti d'ora
- 🕰️ **Orologio** g4 — Trovare l'orologio che segna un'ora
- ✏️ **Ortografia** g4 — Acqua e la sua famiglia (cqu, qu, cu)
- ✏️ **Ortografia** g4 — cia, ce e cie
- ✏️ **Ortografia** g4 — Le doppie
- 📝 **Problemi** g3 — Le parti uguali
- 📝 **Problemi** g3 — Tante volte tanti
- ⚑ 🔢 **Senso del numero** g4 — Indovina il numero
- ⚑ 🔢 **Senso del numero** g5 — Indovina il numero
- 🔢 **Senso del numero** g5 — Le decine
- 🔢 **Senso del numero** g5 — Quanto vale ogni cifra
- ⚑ 🔢 **Senso del numero** g6 — Indovina il numero
- ➡️ **Sequenze** g5 — Il passo che cresce o gira
- ➡️ **Sequenze** g5 — Il ritmo che si ripete

## 63  ·  circa 9 anni

- 📅 **Calendario** g5 — Quanto dura una cosa
- 🗣️ **Coniugazione** g4 — Il futuro
- 🗣️ **Coniugazione** g4 — L'imperfetto
- 🔤 **Grammatica** g4 — Il nome e il suo aggettivo
- 🗺️ **La griglia** g5 — Il perimetro: il giro del bordo
- 📖 **Lessico** g4 — La parola giusta nella frase
- 🧩 **Logica** g4 — La regola applicata dritta
- 🧩 **Logica** g4 — La regola girata: non si può sapere
- 🧩 **Logica** g4 — Quando la regola non è scattata
- 🧩 **Logica** g4 — Quello che la regola non dice
- 📏 **Misure** g3 — Convertire i pesi (kg, g)
- 📏 **Misure** g3 — Convertire le capacità (l, cl, ml)
- 📏 **Misure** g3 — Convertire le lunghezze (km, m, cm)
- ✏️ **Ortografia** g5 — L'accento che cambia il significato (è, sì, là)
- ✏️ **Ortografia** g5 — L'apostrofo
- ✏️ **Ortografia** g5 — Quando ci vuole l'accento
- 📝 **Problemi** g4 — Due conti di fila
- 📝 **Problemi** g4 — Due conti, con i gruppi

## 69  ·  circa 9.5 anni

- 🔤 **Grammatica** g5 — Soggetto e predicato

## 75  ·  circa 10 anni

- 🗣️ **Coniugazione** g5 — Essere o avere nel passato prossimo
- 🗣️ **Coniugazione** g5 — L'imperativo (parla!, andiamo!, non correre!)
- 🗣️ **Coniugazione** g5 — Riconoscere il tempo di un verbo
- 🗣️ **Coniugazione** g5 — Scegliere il tempo che la frase chiede
- 📐 **Geometria** g6 — I cubetti, anche quelli nascosti
- 📐 **Geometria** g6 — Lo sviluppo da piegare
- 🔎 **Indizi** g5 — Chi sono io: le figure
- 🗺️ **La griglia** g6 — Area e perimetro nei rettangoli
- 📖 **Lessico** g5 — I modi di dire
- 🧩 **Logica** g5 — «Nessuno» vuol dire nessuno
- 🧩 **Logica** g5 — Le catene di regole
- 📏 **Misure** g4 — Quale misura è più grande
- 🕰️ **Orologio** g5 — Che ora sarà fra un po'
- 🕰️ **Orologio** g5 — I minuti
- 🕰️ **Orologio** g5 — Le mezze e i quarti d'ora
- 🕰️ **Orologio** g5 — Trovare l'orologio che segna un'ora
- 📝 **Problemi** g5 — Tre conti di fila
- 📝 **Problemi** g5 — Tre conti, con i gruppi
- 🔢 **Senso del numero** g6 — Arrotondare
- 🔢 **Senso del numero** g6 — Circa quanto fa
- 🔢 **Senso del numero** g6 — L'ordine di grandezza, e cosa è impossibile
- ✂️ **Sillabe** g5 — Dove batte la voce

## 81  ·  circa 10.5 anni

- 🗺️ **La griglia** g7 — Area e perimetro, figure qualsiasi
- 📏 **Misure** g5 — I problemi con le misure
- 📝 **Problemi** g6 — I dati che non servono

## 95  ·  circa 11.6 anni

- 🗣️ **Coniugazione** g6 — Essere o avere nel passato prossimo
- 🗣️ **Coniugazione** g6 — I participi irregolari (preso, scritto)
- 🗣️ **Coniugazione** g6 — Il condizionale (vorrei, avrei voluto)
- 🗣️ **Coniugazione** g6 — Il congiuntivo (che io sia, se io fossi)
- 🗣️ **Coniugazione** g6 — Il passato remoto (andò, cossi, mangiammo)
- 🗣️ **Coniugazione** g6 — Il trapassato remoto (ebbi mangiato)
- 🗣️ **Coniugazione** g6 — L'imperativo (parla!, andiamo!, non correre!)
- 🗣️ **Coniugazione** g6 — Riconoscere il tempo di un verbo
- 🗣️ **Coniugazione** g6 — Scegliere il tempo che la frase chiede
- 🗣️ **Coniugazione** g6 — Trapassato prossimo e futuro anteriore (avevo/avrò mangiato)
- ⚑ 📖 **Lessico** g5 — Le parole difficili

---

Totale: 170 classi su 18 moduli.
