...
/* ═══════════════════════════════════════════════════════════════════
   ...
   La scatola ha una testa e un bordo. La testa dice quante volte (o N, se
   è ancora da scegliere) e toccarla apre la scelta del numero; mentre il
   coniglio corre dice **a che giro è** — «3/5» — ed è il pezzo che fa
   vedere un ciclo girare. Se la fila si ferma male dentro un ciclo il
   giro resta scritto, arancione: «sbatte al terzo giro» si legge senza
   leggere. Il bordo, in fondo, mette il cursore subito fuori.
...
import { eApri, volteDi } from '../dati/carte.js'
...
/* il giro di una scatola, se sta girando (o si è fermata lì) */
const giroDi = i => {
  ...
  <template v-for="x in pezzi" :key="x.key">
    <template v-if="x.che === 'posto'">
      ...
      <button v-if="f.s.value.cursore === x.p && f.s.value.fantasma" class="pp-tessera pp-fantasma"
              ...
              :aria-label="eCiclo(f.s.value.consiglio)
                ? `metti qui: ripeti ${volteDi(f.s.value.consiglio)} volte`
                : `metti qui: ${nomeDellaMossa(f.s.value.consiglio)}`"
              @click="f.metti(f.s.value.consiglio)">
        <template v-if="eCiclo(f.s.value.consiglio)">
          <span class="pp-em pp-ciclo-icona">🔁</span><b>{{ volteDi(f.s.value.consiglio) }}</b>
        </template>
    ...
    <button v-else-if="x.che === 'mossa'" class="pp-tessera"
            ...
            @click="f.tocca(x.nodo.i + 1)">
      <Icona :mossa="x.nodo.m" />
    </button>

    <div v-else class="pp-scatola" :class="{ 'pp-dentro': dentro % 2 === 1,
                                             'pp-sospetta': f.s.value.sospette && x.nodo.i >= f.s.value.cursore }"
         :data-scatola="x.nodo.i">
      <button class="pp-testa"
              :class="{ 'pp-manca': x.nodo.volte == null, 'pp-aperta': f.s.value.scelta === x.nodo.i,
                        'pp-brilla': f.s.value.consiglioTesta === x.nodo.i,
                        'pp-gira': !!giroDi(x.nodo.i), 'pp-fermo': !!giroDi(x.nodo.i) && !f.s.value.inCorsa }"
              :data-testa="x.nodo.i" :data-volte="x.nodo.volte ?? 'N'" :data-giro="giroDi(x.nodo.i)"
              :aria-label="x.nodo.volte == null ? 'ripeti: quante volte?' : `ripeti ${x.nodo.volte} volte`"
              @click="f.testa(x.nodo.i)">
        <span class="pp-em pp-ciclo-icona" aria-hidden="true">🔁</span>
        <b>{{ giroDi(x.nodo.i) || (x.nodo.volte ?? 'N') }}</b>
      </button>
