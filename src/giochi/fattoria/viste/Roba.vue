...
function giu(e, v) {
  if (!quantiNe(v.id) && v.prezzo > props.monete) return
...
<template>
  <div class="fa-baule">
    <h2>Il baule</h2>
    <p v-if="categoria === 'animali'">Premi un animale e scegli <b>dove
       farlo arrivare</b>: poi gli dai un nome. Dentro un recinto ci
       resta.</p>
    <p v-else><b>Premi una cosa</b> e scegli dove metterla. Quello che
       hai già è tuo e non si ricompra.</p>
    ...
    <div v-if="categoria === 'animali'" class="fa-griglia">
      <div v-for="a in IN_VENDITA" :key="a.chi"
        ...
        <span :class="['fa-prezzo', { tuo: eMia(a.chi) }]">
          {{ eMia(a.chi) ? 'è tua' : '🪙' + a.prezzo }}</span>
      </div>
    </div>

    <div v-else class="fa-griglia">
