...
const nCol = computed(() => {
  ...
  // +1 sulla lunghezza del secondo numero: il segno vuole una colonna sua,
  // altrimenti in casi come 18+42=60 non resterebbe spazio e sparirebbe
  return Math.max(props.op.colonne, String(props.op.a).length, String(props.op.b).length + 1)
})
const indici = computed(() => Array.from({ length: nCol.value }, (_, i) => nCol.value - 1 - i))

const cifraA = i => cifre(props.op.a)[i]
const cifraB = i => cifre(props.op.b)[i]
const colSegno = computed(() => String(props.op.b).length)   // dove sta il simbolo
...
<template>
  <div class="op">
    ...
    <div v-if="op.tipo !== 'div'" class="griglia" :style="{ '--n': nCol }">
      <!-- primo numero -->
      <div v-for="i in indici" :key="'a'+i" class="cella fissa">{{ cifraA(i) ?? '' }}</div>
      <!-- segno e secondo numero -->
      <div v-for="i in indici" :key="'b'+i" class="cella fissa">
        <span v-if="i === colSegno" class="segno">{{ op.segno }}</span>
        <template v-else>{{ cifraB(i) ?? '' }}</template>
      </div>
...
.griglia { display:grid; grid-template-columns:repeat(var(--n), 1fr);
           gap:3px 4px; font-variant-numeric:tabular-nums;
           width:min(100%, calc(var(--n) * 46px)) }
...
.cella.somma { background:#ffffffcc; box-shadow:inset 0 0 0 2px #e3d6f0; color:var(--viola-scuro) }
.cella.somma.viva { box-shadow:inset 0 0 0 3px var(--viola); background:#efeaff;
...
@keyframes battito { 0%,100%{ background:#efeaff } 50%{ background:#ddd2ff } }
...
.divisore { border-left:3px solid var(--viola-scuro); padding-left:10px; color:var(--viola-scuro) }
...
.tastiera button { padding:10px 0; border-radius:12px; background:#ffffffdd; font-size:20px;
                   font-weight:900; color:var(--viola-scuro); box-shadow:0 3px 0 #ddd0ef }
.tastiera button:active { transform:translateY(2px); box-shadow:0 1px 0 #ddd0ef }
</style>
