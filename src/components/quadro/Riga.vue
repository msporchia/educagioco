...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Icona · nome · una riga di contesto · e a destra o lo stato o il ▶.
   È la stessa riga per un gioco, per un pezzo di scuola e per una
   domanda, ed è **una sola** perché sono lo stesso gesto: guardo una
   cosa e voglio sapere che ne è di lei.
...
<template>
  ...
  <li class="voce-riga" :class="[stato?.cls, { dentro, apribile, aperta: aperto, ultima }]"
    ...
    <!-- `.stop` non è pignoleria: il riquadro intero si apre e si chiude
         ...
         chiesta. -->
    <button v-if="prova" type="button" class="prova" :data-prova="chiave"
...
/* Il ▶ di una riga: piccolo ma con un bersaglio vero sotto — 34 px è il
   minimo che un dito prende senza mirare. */
